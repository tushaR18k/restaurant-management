...
func GetTable() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		err := tableCollection.FindOne(ctx, bson.M{"tabled_id": tableId}).Decode(&table)
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while fetching the table record"})
			return
		}
