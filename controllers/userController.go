...
func SignUp() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		//hash password
		passwaord := HashPassword(*user.Password)
		user.Password = &passwaord
		...
		//create some extra details for the user object- created at, updated_at, ID
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		//conver the login data from postman which is in JSON to golang readable format
		var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
		var user models.User
		...
		//find the user with that email and see if that user even exists
		err := userCollection.FindOne(ctx, bson.M{"email": user.Email}).Decode(&foundUser)
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "user was not found, login seems to be incorrect"})
			return
		}
		...
		// update tokens - token and referesh token
